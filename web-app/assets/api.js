...
const PROVIDER_PRESETS = {
  deepseek: {
    ...
    type: 'openai', defaultModel: 'deepseek-chat', models: ['deepseek-chat', 'deepseek-reasoner', 'deepseek-coder']
  },
  doubao: {
    id: 'doubao', name: '豆包（火山引擎）', apiUrl: 'https://ark.cn-beijing.volces.com/api/v3/chat/completions',
    type: 'openai', defaultModel: 'doubao-pro-32k', models: ['doubao-pro-32k', 'doubao-pro-128k', 'deepseek-r1', 'doubao-1.5-pro']
  },
  qwen: {
    id: 'qwen', name: '千问（阿里云）', apiUrl: 'https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions',
    type: 'openai', defaultModel: 'qwen-plus', models: ['qwen-plus', 'qwen-max', 'qwen-turbo', 'qwen2.5-72b-instruct']
  },
  claude: {
    id: 'claude', name: 'Claude (Anthropic)', apiUrl: 'https://api.anthropic.com/v1/messages',
    type: 'anthropic', defaultModel: 'claude-sonnet-4-6', models: ['claude-sonnet-4-6', 'claude-opus-4-7', 'claude-haiku-4-5-20251001']
  },
  openai: {
    id: 'openai', name: 'OpenAI', apiUrl: 'https://api.openai.com/v1/chat/completions',
    type: 'openai', defaultModel: 'gpt-4o', models: ['gpt-4o', 'gpt-4o-mini', 'gpt-4-turbo', 'o1', 'o1-mini']
  }
};
...
function getResolvedProviderId(stepRole) {
  ...
  if (mappedId && state.providers.some(p => p.id === mappedId && p.apiKey)) return mappedId;
...
// ===================== Core API Call (OpenAI + Anthropic) =====================
async function callLLM(system, messages, { maxTokens = 2048, providerId = null } = {}) {
  ...
  if (provider.type === 'anthropic') {
    return callAnthropicAPI(provider, system, messages, maxTokens);
  }
  return callOpenAIAPI(provider, system, messages, maxTokens);
}

async function callOpenAIAPI(provider, system, messages, maxTokens) {
  const msgs = [];
  ...
  const resp = await fetch(provider.apiUrl, {
    ...
    body: JSON.stringify({ model: provider.model, max_tokens: maxTokens, messages: msgs, temperature: 0.7, stream: false }),
    signal: state.activeAbortController?.signal
  });

  if (!resp.ok) {
    let errMsg;
    try { const errJson = await resp.json(); errMsg = errJson.error?.message || errJson.message || JSON.stringify(errJson); }
...
async function callAnthropicAPI(provider, system, messages, maxTokens) {
  const msgs = (messages || []).map(m => ({ role: m.role, content: m.content }));
  const body = { model: provider.model, max_tokens: maxTokens, messages: msgs, temperature: 0.7 };
  ...
  if (!resp.ok) {
    let errMsg;
    try { const errJson = await resp.json(); errMsg = errJson.error?.message || JSON.stringify(errJson); }
    catch { errMsg = await resp.text(); }
    throw new Error(`Claude API 错误: ${resp.status} - ${errMsg}`);
  }
  const data = await resp.json();
...
function getProviderFormData(providerId) {
  ...
  let model = p.model;
  if (modelValue === '__custom__') model = customInput?.value?.trim() || p.model;
  else if (modelValue) model = modelValue;
  const apiUrl = document.getElementById(`url_${providerId}`)?.value?.trim() || p.apiUrl;
  return { apiUrl, apiKey: document.getElementById(`key_${providerId}`)?.value?.trim() || '', model };
}
...
function saveAllProviders() {
  ...
  try {
    const configToSave = state.providers.map(p => ({ id: p.id, name: p.name, apiUrl: p.apiUrl, model: p.model, type: p.type, apiKey: '' }));
  ...
  try {
    ...
    if (saved) {
      JSON.parse(saved).forEach(p => {
        const existing = state.providers.find(sp => sp.id === p.id);
        if (existing) { if (p.model) existing.model = p.model; if (p.apiUrl) existing.apiUrl = p.apiUrl; }
      });
    }
    const activeId = localStorage.getItem('ai_active_provider');
    if (activeId && state.providers.some(p => p.id === activeId)) state.activeProviderId = activeId;
    const stepMap = localStorage.getItem('ai_step_model_map');
    if (stepMap) Object.assign(state.stepModelMap, JSON.parse(stepMap));
  } catch (e) { /* ignore */ }
}
...
function switchProvider(providerId) {
  const p = getProvider(providerId);
  if (!p) return;
  saveProviderFromForm(state.activeProviderId);
...
function saveSessionData() {
  try {
    const data = {
      userData: Object.fromEntries(Object.entries(state.userData).filter(([k]) => k !== 'customModules' || (state.userData.customModules && state.userData.customModules.length > 0))),
      resume: state.resume, optimizedResume: state.optimizedResume, optimizeCompany: state.optimizeCompany, optimizePosition: state.optimizePosition,
      existingResume: state.existingResume, mode: state.mode, currentStep: state.currentStep, currentResumeSubstep: state.currentResumeSubstep,
      resumeTemplate: state.resumeTemplate, resumeGeneratedOnce: state.resumeGeneratedOnce, resumeLayoutCompressionIndex: state.resumeLayoutCompressionIndex,
      templateConfirmed: state.templateConfirmed, customTemplate: state.customTemplate, templateDesignerChat: (state.templateDesignerChat || []).slice(-20),
      matchChoice: state.matchChoice, matchData: state.matchData, jobChoice: state.jobChoice, jobsFound: state.jobsFound.slice(0, 10),
      jobSearchConfig: state.jobSearchConfig, targetCompany: state.targetCompany, hasStarted: state.hasStarted,
      applications: state.applications.slice(0, 100), savedAt: new Date().toISOString()
    };
...
function loadSessionData() {
  try {
    ...
    if (data.applications && data.applications.length > 0) state.applications = data.applications;
    if (data.hasStarted) state.hasStarted = true;
    return true;
  } catch (e) { try { localStorage.removeItem(SESSION_KEY); } catch (_) {} return false; }
}
...
async function testProviderConnectivity(providerId) {
  ...
  if (statusEl) { statusEl.className = 'api-test-status'; statusEl.textContent = '正在测试连通性...'; }
  try {
    const result = await callLLM('You are a connectivity test assistant. Reply with OK.', [{ role: 'user', content: 'Say OK' }], { maxTokens: 32, providerId });
    const preview = (result || '').replace(/\s+/g, ' ').trim().slice(0, 60);
    if (statusEl) { statusEl.className = 'api-test-status success'; statusEl.textContent = `连接成功${preview ? ' · ' + preview : ''}`; }
  } catch (error) {
    if (statusEl) { statusEl.className = 'api-test-status error'; statusEl.textContent = `连接失败：${error.message}`; }
  }
}
